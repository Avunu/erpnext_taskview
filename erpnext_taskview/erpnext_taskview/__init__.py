...
def update_timesheet_detail(project_name, task_name, status, timesheet_detail):
	...
	timesheet_detail_doc = get_timesheet_detail(project_name, task_name, timesheet_detail.get('name') if timesheet_detail else None)
...
def backend_handler(action, node, update_object):
	...
	try:
		...
		frappe.log_error('Task View Backend Handler', f'{action}\n{node}\n{update_object}')
...
def handle_update(node, update_object):
	...
	# if not node.get('parent').get('isProject') and update_object.get('parent_task'):
...
def handle_toggle_timer(node):
	return update_timesheet_detail(node.get('project'), node.get('docName'), node.get('status'), node.get('timesheet_detail'))
