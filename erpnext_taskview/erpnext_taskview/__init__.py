...
def update_timesheet_detail(project_name, task_name, status, timesheet_detail):
	...
	if status == "running":
		...
		# check if there is a timesheet detail for the current user and the current task
		if timesheet_detail and timesheet_detail.get('name'):
			timesheet_detail_doc = frappe.get_doc('Timesheet Detail', timesheet_detail['name'])
		else:
			# check if there is a timesheet for the current user and the current project that is not submitted
			employee_name = frappe.db.get_value('Employee', {'user_id': frappe.session.user}, 'name')
			timesheet_exists = frappe.db.exists('Timesheet', {'employee': employee_name, 'docstatus': 0, 'project': project_name})
			...
			# otherwise see if the timesheet has a timesheet detail for the current task
			else:
				timesheet = frappe.get_doc('Timesheet', timesheet_exists)
				# just check the timesheet.time_logs for a timesheet detail with the current task
				timesheet_detail_exists = frappe.db.exists('Timesheet Detail', {'parent': timesheet.name, 'task': task_name})
				if not timesheet_detail_exists:
					...
					timesheet_detail_doc = frappe.get_doc('Timesheet Detail', timesheet_detail_exists)
		# resume the timer by setting the start_time to the current timestamp and paused to false
		timesheet_detail_doc.start_time = frappe.utils.now_datetime()
		timesheet_detail_doc.paused = False

	elif status == "paused":
		# pause the timer
		# get the timesheet detail for the current user and the current task
		timesheet_detail_doc = frappe.get_doc('Timesheet Detail', timesheet_detail['name'])
		...
		# convert the total seconds to milliseconds and add it to the from_time
		total_milliseconds = total_seconds * 1000
		timesheet_detail_doc.to_time = frappe.utils.data.get_datetime(from_time.timestamp() + total_milliseconds)

		timesheet_detail_doc.submit()

	if timesheet:
		timesheet.save(ignore_permissions=True)
	if timesheet_detail_doc:
		timesheet_detail_doc.save(ignore_permissions=True)
	frappe.db.commit()

	if not timesheet_detail_doc:
		timesheet_detail_doc = frappe.db.exists('Timesheet Detail', {'parent': timesheet.name, 'task': task_name })
		if timesheet_detail_doc:
			timesheet_detail_doc = frappe.get_doc('Timesheet Detail', timesheet_detail_doc)

	return timesheet_detail_doc.as_dict() if timesheet_detail_doc else None
