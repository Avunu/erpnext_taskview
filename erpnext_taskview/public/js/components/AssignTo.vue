	...
	<div class="assign-to" @click.stop>
		...
		<div class="assign-pills">
			<span v-for="user in assignedUsers" :key="user" class="assign-pill" :title="user">
				...
				<button class="assign-pill-remove" @click="removeUser(user)" title="Unassign">&times;</button>
			</span>
		</div>
		...
		<!-- Dropdown trigger -->
		<button class="task-btn assign-btn--add" @click="toggleDropdown" title="Assign to...">
			+
		</button>

		<!-- Dropdown -->
		<div v-if="dropdownOpen" class="assign-dropdown">
			<input ref="searchInput" v-model="search" class="assign-search" placeholder="Search users..."
				@keydown.esc="closeDropdown" @keydown.stop />
			<ul class="assign-user-list">
				<li v-for="user in filteredUsers" :key="user.value" class="assign-user-item"
					@click="addUser(user.value)">
					<img v-if="avatarUrl(user.value)" class="assign-avatar" :src="avatarUrl(user.value)"
						:alt="user.value" />
					<span v-else class="assign-avatar assign-avatar--initials">{{ initials(user.value) }}</span>
	...
	methods: {
		async loadUsers(): Promise<void> {
			if ((this.$options as any)._cachedUsers) {
				const cached = (this.$options as any)._cachedUsers as { users: UserOption[]; images: Record<string, string> };
		...
		avatarUrl(email: string): string | null {
			return this.userImages[email] || null;
		},
