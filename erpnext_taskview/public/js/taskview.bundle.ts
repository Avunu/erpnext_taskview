...
class TaskViewSortSelector extends (frappe.ui.SortSelector as any) {
  ...
  get_label(fieldname: string): string {
    if (fieldname === "idx") {
      ...
      return (
        (this as any).labels[fieldname] || frappe.meta.get_label((this as any).doctype, fieldname)
      );
    }
  }
}
...
frappe.views.TasksView = class TasksView extends frappe.views.ListView {
  ...
  toggle_result_area() {
    const hasData = (this.taskViewData?.projects?.length ?? 0) > 0 || this.data.length > 0;
